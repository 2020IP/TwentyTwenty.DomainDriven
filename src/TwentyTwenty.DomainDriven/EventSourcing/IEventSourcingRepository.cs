...
namespace TwentyTwenty.DomainDriven.EventSourcing
{
    public interface IEventSourcingRepository<TId>
    {
        ...
        void SaveAndArchive<T>(T aggregate, int? expectedVersion = null) 
            where T : class, IEventSourcingAggregateRoot<TId>, new();
        
        Task SaveAndArchiveAsync<T>(T aggregate, int? expectedVersion = null) 
            where T : class, IEventSourcingAggregateRoot<TId>, new();

        void SaveAndArchive<T>(params T[] aggregates) 
            where T : class, IEventSourcingAggregateRoot<TId>, new();
        
        Task SaveAndArchiveAsync<T>(params T[] aggregates) 
            where T : class, IEventSourcingAggregateRoot<TId>, new();
